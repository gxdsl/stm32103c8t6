...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);	//管教重映射
	...
	TimeBase_Initstructure.TIM_Period = 100 - 1;	//ARR自动重装器的值
	TimeBase_Initstructure.TIM_Prescaler = 720 - 1;	//PSC预分频器的值
	...
	TimOC_Initstructure.TIM_Pulse = 0;	//设置CCR
...
void PWMServo_Init(void)
{
	...
	TimeBase_Initstructure.TIM_ClockDivision = TIM_CKD_DIV1;
	TimeBase_Initstructure.TIM_CounterMode = TIM_CounterMode_Up;
	TimeBase_Initstructure.TIM_Period = 20000 - 1;	//ARR自动重装器的值
	TimeBase_Initstructure.TIM_Prescaler = 72 - 1;	//PSC预分频器的值
	TimeBase_Initstructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(TIM2,&TimeBase_Initstructure);
...
{
	TIM_PrescalerConfig(TIM2,PSC,TIM_PSCReloadMode_Update);	//修改PSC的值
}
